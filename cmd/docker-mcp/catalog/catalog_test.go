...
func TestGetDockerCatalogURL(t *testing.T) {
	...
	t.Run("matches DockerCatalogURLV2 constant when DCR is disabled", func(t *testing.T) {
		url := GetDockerCatalogURL(false)
		assert.Equal(t, DockerCatalogURLV2, url, "should match DockerCatalogURLV2 constant when DCR is disabled")
	})
