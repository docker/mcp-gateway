	...
	tests := []struct {
	...
	}{
		{
			...
			validateServer: func(t *testing.T, servers []db.Server) {
				...
				assert.Equal(t, "test/catalog:latest", servers[0].CatalogSource)
				...
				assert.Equal(t, "test/catalog:latest", servers[1].CatalogSource)
			},
		},
		{
...
func TestAddServersFromCatalogUsingMultipleGlobPatternsWithDuplicates(t *testing.T) {
	dao := setupTestDB(t)
	ctx := t.Context()
